...
Support for Rust development in the Eclipse IDE is provided by the [Eclipse Corrosion](https://github.com/eclipse/corrosion) project. While it currently uses RLS as default, you can successfully configure it so the IDE will use `rust-analyzer` instead. To do so, with an Eclipse IDE where Corrosion is installed, just go to __Window > Preferences > Rust__ and edit the __Path to Rust Language Server__ entry. You'll need to close and reopen all .rs and Cargo files, or to restart the IDE, for this change to take effect.
